...
const ListaTreinos = () => {
  ...
  return (
    <Module title="Treinos Cadastrados">
      <table className="w-full table-auto border-collapse rounded overflow-hidden shadow-md">
        ...
        <tbody>
          {treinos.map((treino) => (
            <tr key={treino.id} className="border-b dark:border-gray-700">
              <td className="p-3">{treino.data}</td>
              <td className="p-3">{treino.atleta}</td>
              <td className="p-3">{treino.atividade}</td>
              <td className="p-3 space-x-3">
                <Link
                  to={`/treinos/cadastrar`}
                  className="text-blue-600 dark:text-blue-400 hover:underline"
                >
                  Cadastrar
                </Link>
                <Link
                  to={`/treinos/editar/${treino.id}`}
                  className="text-blue-600 dark:text-blue-400 hover:underline"
                >
                  Editar
                </Link>
