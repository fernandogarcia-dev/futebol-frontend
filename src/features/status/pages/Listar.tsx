...
const ListaStatus = () => {
  ...
  const handleDelete = (id: string) => {
    setStatus(status.filter((t) => t.id !== id))
  }

  return (
    <Module title="Status Cadastrados">
      <table className="w-full table-auto border-collapse rounded overflow-hidden shadow-md">
        ...
        <tbody>
          {status.map((status) => (
            <tr key={status.id} className="border-b dark:border-gray-700">
              <td className="p-3">{status.data}</td>
              <td className="p-3">{status.atleta}</td>
              <td className="p-3">{status.atividade}</td>
